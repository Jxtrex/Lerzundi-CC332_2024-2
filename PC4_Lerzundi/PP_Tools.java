...
//--------------------------------------------------------
public void CreateDataFileVariable(String FILENAME, int M, int N, char Mode, int[] a, int[] b, int SIZE[]) {
int NUM;
String STR; 
    try {   
        FileWriter FW = new FileWriter(FILENAME);
        for(int i=0;i<=M-1;i++) {
            for(int k=0;k<=N-1;k++) {
                NUM = a[k] + (int)(Math.random()*(b[k]-a[k]+1));
                STR = "" + NUM + "";
                STR = Replicate(' ',SIZE[k]-STR.length()) + STR;
                FW.write(STR);
            }
            FW.write((Mode=='H'?"":"\n"));
        }
